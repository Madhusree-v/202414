...
def save_tables_as_csv(tables, filename):
    ...
    try:
        with open(filename, 'r''w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            for table in tables:
                for row in table:
                    writer.writerow(row)
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        if filename.lower().endswith('.pdf'):
            ...
            with open(json_path, 'r''w', encoding='utf-8') as json_file:
            ...
            csv_path = os.path.join(app.config['UPLOAD_FOLDER'], csv_filename)
            with open(csv_path, 'r''w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
